...
class TrainingConfig:
    ...
    LEARNING_RATE = 2e-4
    MAX_STEPS_STAGE1 = 100  # Basic pretraining
    MAX_STEPS_STAGE2 = 50   # RL GRPO
    MAX_STEPS_STAGE3 = 30   # Knowledge distillation
...
class QuestionsDataPreparer:
    ...
    def __init__(self):
        ...
        try:
            ...
            raise ImportError(f"❌ Real dataset processor not found: {e}. Please ensure real_math_data_processor.py is available.")
        except Exception as e:
            raise RuntimeError(f"❌ Real dataset processor failed to initialize: {e}. Check dependencies and network connection.")

    def prepare_training_data(self, tokenizer) -> Dataset:
        """Prepare training dataset - Using only real math reasoning dataset"""
        print("🔄 Preparing training dataset...")

        # 检查real processor是否可用
        if not self.real_processor:
            raise RuntimeError("❌ Real dataset processor not available. Cannot proceed without real data.")
        ...
        try:
            ...
            raise RuntimeError(f"❌ Failed to prepare real training dataset: {e}. No fallback available.")
...
class QuestionsGenTrainer:
    ...
    def _load_model(self):
        ...
        # Add LoRA adapters (exactly same as reference script)
        ...
        # 修复 Qwen3 attn_bias 错误
        print("🔧 检查并修复 attn_bias...")
        try:
            if hasattr(self.model, 'model') and hasattr(self.model.model, 'layers'):
                for i, layer in enumerate(self.model.model.layers):
                    if hasattr(layer, 'self_attn'):
                        if hasattr(layer.self_attn, 'attn_bias'):
                            if layer.self_attn.attn_bias is None:
                                # 初始化 attn_bias
                                device = next(self.model.parameters()).device
                                dtype = next(self.model.parameters()).dtype
                                layer.self_attn.attn_bias = torch.zeros(1, 1, 1, 1, device=device, dtype=dtype)
    ...
    def stage1_basic_training(self):
        ...
        trainer = SFTTrainer(
            ...
            args = SFTConfig(
                ...
                max_steps = self.config.MAX_STEPS_STAGE1,
                ...
                save_only_model = True, # Use this for WandB etc
            ),
        )
                        ...
                        if hasattr(layer, 'self_attn'):
                            if hasattr(layer.self_attn, 'attn_bias'):
                                if layer.self_attn.attn_bias is None:
                                    ...
                                    dtype = next(self.model.parameters()).dtype
                                    layer.self_attn.attn_bias = torch.zeros(1, 1, 1, 1, device=device, dtype=dtype)
                                    print(f"✅ 恢复过程中修复 layer {i} attn_bias")
                print("✅ 恢复过程中 attn_bias 检查完成")
    ...
    def stage2_grpo_training(self):
        ...
        try:
            ...
            raise RuntimeError(f"❌ Failed to load variation training data: {e}. Cannot proceed without real data.")
